...
public class OperativeOldAdapter extends RecyclerView.Adapter<OperativeOldAdapter.MyViewHolder> {

    Context context;
    ArrayList<OperativeReportOldModel.Data> dataArrayList;

    public OperativeOldAdapter(Context context, ArrayList<OperativeReportOldModel.Data> dataArrayList) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        OperativeReportOldModel.Data data=dataArrayList.get(position);
        holder.tv_report_examination.setText(data.getCreatedBy());
//        holder.tv_date.setText(data.getCreatedDate());
        ...
        holder.tv_view_report.setOnClickListener(v -> {
            ...
            String pdfUrl=new UrlWithURLDecoder().getOperative_report_pdf_url(String.valueOf(data.getId()),
                    "1", mrn,data.getReportType(), String.valueOf(hosp));

            Intent intent=new Intent(context, PdfWebViewActivity.class);
            intent.putExtra("url",pdfUrl);
            intent.putExtra("name",data.getCreatedBy()+" Operative Report");
