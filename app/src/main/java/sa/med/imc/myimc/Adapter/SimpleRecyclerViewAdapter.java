...
public class SimpleRecyclerViewAdapter extends RecyclerView.Adapter<SimpleRecyclerViewAdapter.Viewholder> {
    ...
    List<String> list, messagefilkterdlist;
    String lastValue = "";
    String lang = Constants.ENGLISH;

    public SimpleRecyclerViewAdapter(Context context, List<String> list, String lastValue) {
    ...
    public void onBindViewHolder(@NonNull Viewholder holder, final int position) {
        ...
        if (lastValue.length() > 0) {
            if (lastValue.equalsIgnoreCase(list.get(position))) {
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    for (String row : messagefilkterdlist) {

                        // name match condition. this might differ depending on your requirement
                        // here we are looking for name or phone number match
                        if (row.toLowerCase(Locale.getDefault()).contains(charString.toLowerCase())) {
